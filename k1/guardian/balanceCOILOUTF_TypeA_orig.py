...
def CheckSuspension(optic,logger):
    ...
    warningflag = True
    while not ezca['GRD-VIS_%s_STATE'%optic] == 'TWR_DAMPED' or not ezca['GRD-VIS_%s_STATE'%optic] == 'MN_COIL_BALANCING':
        if warningflag:
            now_state = ezca['GRD-VIS_%s_STATE'%optic]
            logger.debug('Guardian is not in TWR_DAMPED. Now is %s, Please request TWR_DAMPED'%(now_state))
            warningflag = False
...
e.g. balanceCOILOUTF.py ITMY
     balanceCOILOUTF.py ITMY -f 0.3 -a 1000 -d 10 -r 10''',formatter_class=argparse.RawTextHelpFormatter)
    ...
    try:
        for stage in stages:
            for coil in coils:
                gg,avgI,avgQ,stdI,stdQ = Balancing(optic,stage,coil,freq,oscAMP,sweeprange,logger,Np=Npoints,avgDuration=duration,settleDuration=SettleDuration)
