############################################
# ITMX gardian
# copied from ETMX by YF pm June 30th 2019.
############################################
import time
import math
...
#  	## COMB ##
#        for DOF in ['P08']:
#            ezca['VIS-'+optic+'_PAY_OLSERVO_DAMP_%s_TRAMP'%DOF] = 10.0
#            ezca['VIS-'+optic+'_PAY_OLSERVO_DAMP_%s_RSET'%DOF] = 2
#            ezca['VIS-'+optic+'_PAY_OLSERVO_DAMP_%s_GAIN'%DOF] = 1.0
        ...
        for ii in range(24):
            lib.config_BPCOMB_from_description(optic = optic, DOFNUM = ii+1,
                                           onSW = [1,2,3],TRAMP = TRAMP, LIMIT = 20000)
	return True
...
class ENGAGE_PAY_DAMPING(GuardState):
    ...
    def main(self):
        TRAMP = 10
        log('Turning on the damping filters')
        ezca.switch('VIS-' + optic + '_IM_TMOLDAMP_P','FM1','FM2','FM3','FM4','FM5','FM6','FM7','LIMIT','INPUT','OUTPUT','ON','FM8','FM9','FM10','OFF')
