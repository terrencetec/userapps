...
from burt import *
# In order to test this file in fake vistool and fake ezca the line below has to be changed.
...
class PAYLOAD_TRIPPED(GuardState):
    """We end up at the TRIPPED state if any other state notices
	that one of the WDs has tripped. We ramp things down (but not 
	instantaneously in case it's just one of the BIO WDs and 
	other things are live), and then proceed to SAFE when the WDs have been reset.
    ...
    def main(self):
        global visObj
        checkvis() # check manually here rather than in watchdog_check decorator
        notify('PAYLOAD_TRIPPED state, main()')
...
### The code below was written by Fabian ###
### Apparently the largest number in previously defined states is 161.

class OBSERVATION(GuardState):
    """
    In this state:
        IM bandpass damping on,
        IM velocity damping off,
        BF velocity damping off,
        SF velocity damping off,
        OPLEV velocity damping off (TBD by commissioners),
        OPLEV DC control off (TBD by commissioners).
    In case the BF or SF trips in the process the system should go directly to the usual TRIPPED state.
    In case the payload trips it should go to ENGAGING_PAYLOAD_TRIPPED and from there to PAYLOAD_TRIPPED.
    """
    index = 200
    @watchdog_check
    def main(self):
        ...
        global state
        state = 'OBSERVATION'
        notify('Suspension in OBSERVATION state')
        return True
...
class ENGAGING_OBSERVATION(GuardState):
    ...
    In case the BF or SF trips in the process the system should go directly to the usual TRIPPED state.
    In case the payload trips it should go to ENGAGING_PAYLOAD_TRIPPED and from there to PAYLOAD_TRIPPED.
    The configuration of the OBSERVATION state is loaded from a snapshot, which happens in the following sequence:
        beginControlSequence(stateObj,level) --> rampSetpoint(level) -->  val=valFromSnap(level,'SET',DOF,setSnapDict[level],'OFFSET')
    """
    index = 205
    request = False
    @watchdog_check # Apparently this watchdog check cannot be inside of any function. It has to be here and Guardian keeps checking in a loop.
    def main(self)
        global visObj
        global state
        state = 'OBSERVATION'
        notify('Engaging OBSERVATION state, please hold your breath.')
        # The following if clauses turn off the filters in BF, F1, TM and IM.
        if(disengagedControlSequence(self,level=['TM','IM']) == 'TRIPPED'):
            return 'PAYLOAD_TRIPPED'
        if(disengagedControlSequence(self,level=['BF','F1']) == 'TRIPPED'):
            return 'TRIPPED'
        # The following clauses turn on the filters in BF, F1, TM and IM.
        if(beginControlSequence(self,level='PAYLOAD')) == 'TRIPPED'):
            return 'ENGAGING_PAYLOAD_TRIPPED'
        if(beginControlSequence(self,level=['TM','IM'])) == 'TRIPPED'):
            return 'ENGAGING_PAYLOAD_TRIPPED'
        return(True)
...
class DISENGAGING_OBSERVATION(GuardState):
    ...
    def main(self)
        ...
        global state
        state = 'OBSERVATION'
        notify('Disengaging OBSERVATION state and returning to ALIGNED state')
        return disengageControlSequence(self,level='PAYLOAD')

class ENGAGING_PAYLOAD_TRIPPED(GuardState):
    """
    This process restores the BF and F1 velocity broadband filters and then goes to PAYLOAD_TRIPPED
    """
    index = 215
    request = False
    @watchdog_check
    def main(self)
        ...
        global state
        state = 'OBSERVATION'
        notify('Engaging BF and SF velocity damping filters')
        if(beginControlSequence(self,level=['BF','SF'])) == 'TRIPPED'):
            return 'TRIPPED'
        return(True)
...
('INIT','SAFE'),
('TRIPPED','SAFE'),
('PAYLOAD_TRIPPED','ENGAGING_IM_DAMP'),
